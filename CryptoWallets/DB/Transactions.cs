//------------------------------------------------------------------------------
// Clase Transactions generada automáticamente con CrearClaseSQL
// de la tabla 'Transactions' de la base 'CryptoWallets'
// Fecha: 24/Jan/2025 00:34:20
//
...
public class Transactions{
    ...
    public static DataTable Tabla(string sel){
        // devuelve una tabla con los datos de la tabla Transactions
        SqlDataAdapter da;
    ...
    ///  se comprueba si el TransactionID existe en la tabla.
    ///  TODO: Si en lugar de TransactionID usas otro campo, indicalo en la cadena SELECT
    ///  También puedes usar la sobrecarga en la que se indica la cadena SELECT a usar
    /// </summary>
    public override string Actualizar(){
        // TODO: Poner aquí la selección a realizar para acceder a este registro
        //       yo uso el TransactionID que es el identificador único de cada registro
    ...
    public override string Actualizar(string sel){
        ...
        ////       Yo compruebo que sea un campo llamado TransactionID, pero en tu caso puede ser otro
        ////       Ese campo, (en mi caso TransactionID) será el que hay que poner al final junto al WHERE.
        //sCommand = "UPDATE Transactions SET CryptoID = @CryptoID, Name = @Name, Abbreviation = @Abbreviation, Description = @Description, CreatedAt = @CreatedAt, UpdatedAt = @UpdatedAt, DeletedAt = @DeletedAt, AppID = @AppID, Platform = @Platform,  ! es = @ ! es, WalletID = @WalletID, WalletName = @WalletName, ApplicationID = @ApplicationID, AddressID = @AddressID, PublicKey = @PublicKey, PrivateKey = @PrivateKey, SeedID = @SeedID, Phrase = @Phrase, ID = @ID, Amount = @Amount, TransactionDate = @TransactionDate  WHERE (TransactionID = @TransactionID)";
    ...
    public override string Crear(){
        ...
        ////       Yo compruebo que sea un campo llamado TransactionID, pero en tu caso puede ser otro
        //sCommand = "INSERT INTO Transactions (Name, Abbreviation, Description, CreatedAt, UpdatedAt, DeletedAt, Platform,  ! es, WalletName, ApplicationID, PublicKey, PrivateKey, Phrase, Amount, TransactionDate)  VALUES(@Name, @Abbreviation, @Description, @CreatedAt, @UpdatedAt, @DeletedAt, @Platform, @ ! es, @WalletName, @ApplicationID, @PublicKey, @PrivateKey, @Phrase, @Amount, @TransactionDate) ";
    ...
    /// <summary>
    ///  Devuelve una colección de Transactions según la cadena select.
    /// </summary>
    public static List(Of Transactions) TablaCol(string sel){
        List(Of Transactions) col = new List(Of Transactions);
    ...
    public static Transactions Reader2Tipo(SqlDataReader r){
        ...
        o_Transactions.Name = r["Name"].ToString();
        o_Transactions.Abbreviation = r["Abbreviation"].ToString();
        o_Transactions.Description = r["Description"].ToString();
        o_Transactions.CreatedAt = ConversorTipos.DateTimeData(r["CreatedAt"].ToString());
        o_Transactions.UpdatedAt = ConversorTipos.DateTimeData(r["UpdatedAt"].ToString());
        o_Transactions.DeletedAt = ConversorTipos.DateTimeData(r["DeletedAt"].ToString());
        o_Transactions.AppID = ConversorTipos.Int32Data(r["AppID"].ToString());
        o_Transactions.Platform = r["Platform"].ToString();
        o_Transactions.Notes = r[" ! es"].ToString();
        o_Transactions.WalletID = ConversorTipos.Int32Data(r["WalletID"].ToString());
        o_Transactions.WalletName = r["WalletName"].ToString();
        o_Transactions.ApplicationID = ConversorTipos.Int32Data(r["ApplicationID"].ToString());
        o_Transactions.AddressID = ConversorTipos.Int32Data(r["AddressID"].ToString());
        o_Transactions.PublicKey = r["PublicKey"].ToString();
        //o_Transactions.PrivateKey = r["PrivateKey"];
        o_Transactions.SeedID = ConversorTipos.Int32Data(r["SeedID"].ToString());
        o_Transactions.Phrase = r["Phrase"].ToString();
        o_Transactions.ID = ConversorTipos.Int32Data(r["ID"].ToString());
